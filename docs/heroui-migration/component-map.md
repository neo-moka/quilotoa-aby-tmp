...
| `emoji-reaction-button` | **Adoptado** — por pedido de producto, asumiendo los costos que el descarte había medido | El descarte previo era correcto en sus hechos y los hechos no cambiaron (el gap test sigue en verde); cambió el motivo: se adoptó para unificar los pills del chat con el estilo Pro (press-scale, estado seleccionado con acento, ramp BEM), no porque `isReadOnly` liberara el `<span>`. Mitigaciones en `MessageReactions.tsx`: el `<span>` que delega hover/focus **se queda** — el popover "quién reaccionó" cuelga de él, así que el `pointer-events:none` del read-only no lo rompe; `HERO_ACCENT_SCOPE` en el root (selected border/tint/count leen `var(--accent)` bare — sin el scope pintan el gris hover, y el conteo sale invisible) y `HERO_MUTED_SCOPE` en `.Count` (una lectura bare de `var(--muted)`); geometría pisada con `h-7` en el root (`--sm` mide 24px por padding y el contrato del chat es 28px) más `flex items-center` en `.Emoji` para centrar el `<img>` de emoji custom que la línea base hunde; `title` va en el glifo (`EmojiGlyph`), no en el root. Su CSS entra por `heroui.css` (`emoji-reaction-button.css`, sin componente base; 7 lecturas bare de `--accent`, 1 de `--muted` en dist — cubiertas por los scopes). `emojiReactionButtonHeroUiGap.test.mjs` se conserva con rol invertido: ya no custodia un descarte sino los costos que la adopción paga; si empieza a fallar, upstream los eliminó y las mitigaciones sobran. |
