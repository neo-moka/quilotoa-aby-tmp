...
|---|---|---|---|
| Relay | `Dockerfile` (targets `runtime` y `runtime-debug`) | `ghcr.io/neo-moka/buzz` | `.github/workflows/docker.yml` |
| Push gateway | `Dockerfile.push-gateway` | `ghcr.io/neo-moka/buzz-push-gateway` | `.github/workflows/docker.yml` |
| Sprig (runtime de agentes) | `Dockerfile.sprig` | `ghcr.io/neo-moka/buzz-sprig` | `.github/workflows/sprig-image.yml` |
...
Las tres se consultan con el patrón `vars.X != '' && vars.X || '<default de
upstream>'`, así que **si falta alguna el workflow no falla: publica al
namespace de Block y el push muere con 403.** Tienen que estar las tres.

| Variable | Valor |
|---|---|
| `GHCR_IMAGE` | `ghcr.io/neo-moka/buzz` |
| `GHCR_PUSH_GATEWAY_IMAGE` | `ghcr.io/neo-moka/buzz-push-gateway` |
| `GHCR_SPRIG_IMAGE` | `ghcr.io/neo-moka/buzz-sprig` |

```bash
gh variable set GHCR_IMAGE              -R neo-moka/buzz -b ghcr.io/neo-moka/buzz
gh variable set GHCR_PUSH_GATEWAY_IMAGE -R neo-moka/buzz -b ghcr.io/neo-moka/buzz-push-gateway
gh variable set GHCR_SPRIG_IMAGE        -R neo-moka/buzz -b ghcr.io/neo-moka/buzz-sprig
```

`GHCR_IMAGE` y `GHCR_SPRIG_IMAGE` ya venían de upstream.
`GHCR_PUSH_GATEWAY_IMAGE` la agrega este fork, porque el gateway era el único
de los tres que estaba hardcodeado a `ghcr.io/block`.
...
1. **Habilitar Actions en el fork.** GitHub no registra los workflows de un
   ...
   Mientras no se haga, `gh api /repos/neo-moka/buzz/actions/workflows` devuelve
...
Si algún día `buzz` se da de alta como producto de la plataforma —proyecto GCP,
`cb-deploy@`, landing zone en `live/gcp/develop/buzz/`— la decisión de dónde
viven estas imágenes se vuelve a abrir: el contrato manda las imágenes de
aplicación al Artifact Registry del proyecto del producto. Hoy `buzz` no está
declarado en `stacks/github-org` ni tiene proyecto, así que esa pregunta no
está planteada.
