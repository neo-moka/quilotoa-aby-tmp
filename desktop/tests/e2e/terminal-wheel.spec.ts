...
const TERM = 'section[aria-label="ABY Term"]';
const NAMED = 0x0100_0000;
const FG = NAMED | 256;
const BG = NAMED | 257;

/**
 * The shipped mock bridge throws on every `terminal_*` command, so ABY Term is
...
async function reveal(page: Page) {
  ...
  // ABY Term needs a channel: TerminalBootstrap's context is null on Home, so
...
test("project terminal button opens ABY Term for the repository", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1280, height: 800 });
  await installTerminalBackend(page);
...
test("scrollback: wheel over ABY Term reaches terminal_scroll", async ({
  page,
}) => {
  await reveal(page);
...
test("concealed terminal viewport does not steal ABY focus", async ({
  page,
}) => {
  await reveal(page);
  await page.keyboard.press("Meta+j");
