...
test("app font size and conversation density apply independently", async ({
  ...
  await expect(densityDescription).toHaveText(
    "Spacing in conversations and Markdown content across ABY",
  );
...
test("appearance picker — system tab (ABY follows OS)", async ({ page }) => {
  await seedTheme(page, "buzz");
  await installMockBridge(page);
...
test("appearance picker — light tab (ABY)", async ({ page }) => {
  await seedTheme(page, "buzz");
  await installMockBridge(page);
...
test("appearance picker — dark tab (ABY Dark)", async ({ page }) => {
  await seedTheme(page, "buzz-dark");
  await installMockBridge(page);
...
test("settings nav uses ABY active pill + hover (light)", async ({ page }) => {
  await seedTheme(page, "buzz");
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.getByTestId("open-settings").click();
  ...
  // Appearance is the active section here; its nav row should carry the ABY
...
test("settings nav uses ABY active pill + hover (dark)", async ({ page }) => {
  await seedTheme(page, "buzz-dark");
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.getByTestId("open-settings").click();
...
for (const { mode, theme } of [
  ...
  test(`${theme} ignores the ABY prominent preference`, async ({ page }) => {
    await seedTheme(page, theme);
    await page.addInitScript(
      ({ key }) => window.localStorage.setItem(key, "true"),
      { key: PROMINENT_ACTIVE_TAB_STORAGE_KEY },
...
test("appearance hides accent picker under ABY", async ({ page }) => {
  await seedTheme(page, "buzz");
  await installMockBridge(page);
  const panel = await openAppearance(page, "light");
  // The accent picker is hidden while a ABY theme is active. Its neutral
...
test("non-ABY glass preserves the selected theme sidebar tint", async ({
  page,
}) => {
...
test("accent picker reveals/hides when toggling ABY", async ({ page }) => {
  // Start on a non-ABY theme so the accent picker is present, then select the
  // ABY tile — the picker should animate out and unmount. Reselecting a
  // non-ABY tile brings it back. Asserts the presence toggle (the motion
  ...
  // Switch to ABY — picker should leave (allow the exit animation to settle).
  ...
  // Back to a non-ABY theme — picker returns.
...
test("ABY light and dark modes apply live without a reload", async ({
  page,
}) => {
  await seedTheme(page, "buzz");
  await installMockBridge(page);
...
test("ABY follows native system theme changes without a reload", async ({
  page,
}) => {
  await seedTheme(page, "buzz");
  await page.addInitScript(() => {
