...
const CODEX_NOT_INSTALLED = {
  ...
  default_args: [],
  mcp_command: null,
  install_hint: "ABY talks to Codex through the Codex CLI.",
...
test.describe("Doctor panel state screenshots", () => {
  ...
  test("00-runtime-card-layout", async ({ page }) => {
    ...
    await expect(page.getByTestId("doctor-runtime-codex")).not.toContainText(
      "ABY talks to Codex through the Codex CLI.",
    );
        ...
        },
        {
          ...
          steps: [
            ...
            },
            {
              ...
              stdout: "",
              stderr:
                "The installer finished, but ABY still could not use codex (observed: NotInstalled).",
              exit_code: null,
              hint: "ABY requires the vendor CLI executable, not only its desktop app. If the CLI was installed while ABY was open, restart ABY and check again.",
            },
          ],
        },
      ],
  ...
  test("11-outdated-adapter-warning", async ({ page }) => {
    ...
    await expect(dialog).toContainText(
      "Older ABY releases using the legacy adapter may lose community access",
    );
