...
export function getBuzzToolInfo(title: string): BuzzToolInfo | null {
  ...
  if (name.includes("workflow") || name === "approve_step") {
    return {
      icon: Workflow,
      label: isRead
        ? "Reads workflow state from ABY."
        : "Updates workflow state in ABY.",
  ...
  if (
    ...
    return {
      icon: Hash,
      label: isRead
        ? "Reads channel context from the ABY relay."
        : "Changes channel state in the ABY relay.",
      tone: isWrite ? "write" : "read",
    };
  }
  if (
    ...
    return {
      icon: Users,
      label: isRead
        ? "Reads ABY identity or presence data."
        : "Updates ABY identity or membership data.",
  ...
  if (name.includes("search") || name === "get_feed") {
    return {
      icon: Search,
      label: "Searches relay-visible ABY history.",
      tone: "read",
    };
  }
  if (
    ...
    return {
      icon: Send,
      label: "Publishes relay-visible ABY activity.",
      tone: "write",
    };
  }
  ...
  return {
    icon: MessageSquare,
    label: isRead ? "Reads from ABY." : "Writes to ABY.",
    tone: isWrite ? "write" : "read",
  };
}
