...
/** ABY navigation can use either its production tint or a stronger tab. */
...
/**
 * Build the sample thumbnail as an SVG data URL from the ABY gradient
 * tokens. Data-URL images cannot resolve CSS variables, so the token values
 * are read from the live stylesheet and baked in per render — if the ABY
...
function buzzGradientSampleImage(isDark: boolean): string {
  ...
  const top = isDark
    ? readToken("--buzz-gradient-dark-top", "#2d164b")
    : readToken("--buzz-gradient-light-top", "#ccbae3");
  const bottom = isDark
...
function ThreadLayoutDiagram({ mode }: { mode: ThreadViewMode }) {
  ...
  // ABY gradient tokens directly and follows theme.css automatically.
  const gradientTop = isDark
    ? "var(--buzz-gradient-dark-top, #2d164b)"
    : "var(--buzz-gradient-light-top, #ccbae3)";
    ...
    <SettingsOptionRow className="items-start">
      <div className="min-w-0">
        ...
        <p
          ...
          data-settings-subcopy
        >
          Choose the highlight color used throughout ABY.
        </p>
      </div>
      <div
