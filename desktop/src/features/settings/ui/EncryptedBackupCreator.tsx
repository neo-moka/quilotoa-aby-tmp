      ...
      >
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          {/* Pro's `NativeSelect` keeps the real `<select>` — `Trigger` spreads
              ...
              node — and swaps the OS-drawn arrow for a themed chevron. The
              native arrow is painted by system chrome and ignored the app's
              palette, which shows on this dark popover surface. */}
          <NativeSelect>
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent className="max-w-lg" data-testid="encrypted-backup-dialog">
        <DialogHeader className="pr-8">
          ...
          <DialogDescription>
            You can close this window while ABY finishes the backup in the
            background.
          </DialogDescription>
        </DialogHeader>
        <div
        ...
        >
          ...
          {!state.downloadPending && !state.savedPassword ? (
            <p className="text-xs leading-5 text-muted-foreground">
              Keep the file private and save its password somewhere safe — ABY
