 ...
 * default config, community choice, community profile. Password backup is an
 * optional subview of identity/key, not another position.
 */
export const TOTAL_ONBOARDING_PAGES = 6;

/**
 * Shared pill shape (38px tall) for every onboarding primary CTA. The
 * `buzz-onboarding-cta` marker is what the dark shell hangs its element border
 * on — bordering every `button` instead would box inline text links too.
 */
const ONBOARDING_CTA_SHAPE =
  "buzz-onboarding-cta h-[2.375rem] rounded-full px-6";
...
/**
 * Primary-CTA styling for the landing screen. It used to carry the page's own
 * background color as its label, back when the pill was a light slab on a
 * bright field. The pill is now the dark element surface, so it takes the same
 * label token as the in-step CTAs — echoing the background here would print
 * near-black text on a near-black button.
 */
...
export const ONBOARDING_SECONDARY_CTA_CLASS =
  "buzz-onboarding-cta h-9 rounded-full bg-foreground/10 px-6 text-foreground hover:bg-foreground/15 hover:text-foreground";

/**
 * Icon-control styling for onboarding surfaces that sit on the textured card:
 * violet backup ink (`--buzz-onboarding-backup-ink`) with a plain
