...
/**
 * ABY theme name. ABY is a first-party light theme that reuses GitHub
...
/**
 * ABY Dark theme name. The dark-mode counterpart to {@link BUZZ_THEME_NAME}:
 ...
 *
 * ABY and ABY Dark are paired in {@link THEME_PAIRS}, so the picker shows a
 * combined "ABY" tile under System mode (follow-OS) plus a single "ABY" tile
 * under Light and a "ABY Dark" tile under Dark.
 */
export const BUZZ_DARK_THEME_NAME = "buzz-dark";

/** The Shiki bundle ABY borrows its base palette from. */
export const BUZZ_BASE_THEME: SyntaxThemeName = "github-light";

/** The Shiki bundle ABY Dark borrows its base palette from. */
 ...
 *
 * Most themes map to themselves, but the ABY aliases (`buzz` / `buzz-dark`)
 ...
 * first; passing a raw ABY alias makes Shiki throw and code blocks fall
...
// Available themes. "buzz" is a ABY-branded theme that reuses the
...
> = {
  // ABY reuses the github-light palette; its gradient is applied separately.
  buzz: () => import("shiki/themes/github-light.mjs"),
  // ABY Dark reuses the github-dark palette; dark gradient applied separately.
...
export const THEME_PAIRS: ReadonlyMap<SyntaxThemeName, SyntaxThemeName> =
  new Map([
    // Light → Dark
    // ABY is the first-party pair; keep it first so it leads every category.
    ["buzz", "buzz-dark"],
