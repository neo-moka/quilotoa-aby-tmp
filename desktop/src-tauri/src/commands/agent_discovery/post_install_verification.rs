...
pub(super) fn run(
    ...
    // Observe PATH changes and binaries added after ABY launched.
...
fn failure(
    ...
    Some(InstallStepResult {
        ...
        stdout: String::new(),
        stderr: format!(
            "The installer finished, but ABY still could not use {runtime_id} (observed: {observed})."
        ),
        exit_code: None,
        hint: Some(
            "ABY requires the vendor CLI executable, not only its desktop app. If the CLI was installed while ABY was open, restart ABY and check again."
                .to_string(),
        ),
    })
}
