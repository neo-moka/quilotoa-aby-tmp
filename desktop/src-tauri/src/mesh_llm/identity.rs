...
//! ABY/Nostr identity). The node presents a signed ownership attestation
...
//! has one owner identity whether mesh runs embedded in ABY or standalone.
...
impl OwnerIdentity {
    /// Sign a ABY-to-MeshLLM ownership binding. The member's Nostr signature
