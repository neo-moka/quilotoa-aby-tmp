//! ABY Nest — persistent agent workspace at `~/.buzz`.
//!
//! Creates a shared knowledge directory on first launch so every
//! ABY-spawned agent starts with orientation (AGENTS.md) and a
...
/// Creates the ABY nest at `~/.buzz` if it doesn't already exist.
///
...
/// Creates a ABY nest at the given `root` path.
///
